		...
		<div class="container">
			<div class="row pad-botm">
				<div class="col-md-12">
					...
					<strong>Each question have 4 marks.<strong>&nbsp;<a href="checkStudentList.jsp">Back</a>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<%
						...
						while (resultSet.next()) {
							...
							ResultSet rsActualAnswer = DatabaseConnection.getResultFromSqlQuery("select * from questions where id='" + questionID + "'");
							if (rsActualAnswer.next()) {
								examTopic=rsActualAnswer.getString("subjectTopic");;
								answer = rsActualAnswer.getString("answer");
							}
							if (answer.equals(hitAnswer)) {
								resultCorrect++;
								testMarks = resultCorrect * 4;
							} else {
								resultWrong++;
	
							}
							}
							ResultSet rsStudentName = DatabaseConnection.getResultFromSqlQuery("select studentName from student where id='" + id + "'");
							if (rsStudentName.next()) {
								studentName = rsStudentName.getString("studentName");
							}
							int resultReport = DatabaseConnection.insertUpdateFromSqlQuery("insert into tblresultreport(student_id,student_name,subjectTopic,correct_answer,wrong_answer,test_marks,examtime)values('"+ id + "','" + studentName + "','"+examTopic+"','" + resultCorrect + "','" + resultWrong + "','" + testMarks+ "','" + resultSet.getTimestamp("examdate") + "')");
						}
					%>
					<div class="col-md-12">
							...
							<div class="panel-body">
								<div class="table-responsive">
									<table class="table">
										...
										<tbody>
											<tr class="success">
												<td><%=studentResultReport.getInt("student_id")%></td>
												<td><%=studentResultReport.getString("student_name")%></td>
												<td><%=studentResultReport.getString("subjectTopic")%></td>
												<td><%=studentResultReport.getString("correct_answer")%></td>
												<td><%=studentResultReport.getString("wrong_answer")%></td>
												<td><%=studentResultReport.getString("test_marks")%></td>
												<td><%=studentResultReport.getString("examtime")%></td>
											</tr>
										</tbody>
										<%
											}
